...
local mobdebug = {
  ...
  connecttimeout = 0.02, -- connect timeout (s)
}
...
local function s(t, opts)
  ...
  local space, maxl = (opts.compact and '' or ' '), (opts.maxlevel or 4)
...
local function get_basename(file)
  local nStart = string.find(file, "/[^/]*$")
  if(nStart) then
    return string.sub(file, nStart + 1)
  else
    return file
  end
end
...
local function stack(start)
  ...
  for i = (start or 0), 100 do
    ...
    table.insert(stack, { -- remove basedir from source
      {source.name, get_basename(removebasedir(src, basedir)),
...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    if (lastsource ~= caller.source) then
      ...
      -- Get file name only, to support Simple Lua
...
local max_str_size = 4000 -- node js subprocess stdin buffer limit
local function myout( str )
  io.stdout:write(str .. "\n")
  io.stdout:flush()
end
...
local function handle(params, client, options)
  ...
  or command == "over" or command == "exit" then
    ...
    while true do
      ...
      elseif status == "202" then
        ...
        if file and line then
          myout(json.encode({MsgType="Paused", File=file, Line=line}))
        end
      elseif status == "203" then
        _, _, file, line, watch_idx = string.find(breakpoint, "^203 Paused%s+(.-)%s+(%d+)%s+(%d+)%s*$")
        if file and line and watch_idx then
          myout("Paused at file " .. file .. " line " .. line .. " (watch expression " .. watch_idx .. ": [" .. watches[watch_idx] .. "])")
        end
  ...
  elseif command == "setb" or command == "asetb" then
    ...
    if file and line then
      ...
      if command == "asetb" or client:receive() == "200 OK" then
        ...
        myout("setb successfully")
      else
        print("Error: breakpoint not inserted")
      end
    else
      myout("Invalid command")
    end
  elseif command == "setw" then
      ...
      while true do
        ...
        if status == "200" then
          ...
          if len > 0 then
            ...
            if value_str:len() > max_str_size then
              value_str = value_str:sub(0, max_str_size)
            end
            myout(json.encode({MsgType="Eval", Exp=string.sub(exp, string.len("return  ")), Value=value_str}))
            return res[1], res
          end
        ...
        elseif status == "401" then
          ...
          myout(json.encode({MsgType="Eval", Exp=string.sub(exp, string.len("return  ")), Value="Error in expression"}))
  ...
  elseif command == "stack" then
    ...
    if status == "200" then
      ...
      local stackframe_lines = {}
      for _,frame in ipairs(stack) do
        table.insert(stackframe_lines, frame[1])
      end
      myout(json.encode({MsgType="Stack", Frames=stackframe_lines}))
      return stack
    elseif status == "401" then
...
local function listen(host, port)
  ...
  mylog("Lua Remote Debugger")
  mylog("Run the program you wish to debug")
  ...
  -- if file and line then
  --   myout("Paused at file " .. file )
  myout("Type 'help' for commands")
  -- else
  --   local _, _, size = string.find(breakpoint, "^401 Error in Execution (%d+)%s*$")
  --   if size then
  --     mylog("Error in remote application: ")
  --     mylog(client:receive(size))
  --   end
  -- end
