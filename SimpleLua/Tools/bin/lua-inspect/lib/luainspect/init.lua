...
local ops = {}
ops['shift_r'] = function(_1,_2) return _1 end -- chenliang3
ops['shift_l'] = function(_1,_2) return _1 end -- chenliang3
ops['bit_and'] = function(_1,_2) return _1 end -- chenliang3
ops['bit_or'] = function(_1,_2) return _1 end -- chenliang3
...
function M.require_inspect(name, report, spath)
    ...
    if msrc then
        ...
        if mast then
            ...
            M.inspect(mast, mtokenlist, msrc, report, 1)
...
local function copytable(table, nDeep)
    nDeep = nDeep or 1
    local newtable = {}
    for k,v in pairs(table) do
        if type(v) == 'table' then
            if (nDeep < 3) then
                rawset(newtable, k, copytable(v, nDeep + 1))
            end
        else
            rawset(newtable, k, v)
        end
...
local function IsBasicTable(tb, nDeep)
    nDeep = nDeep or 1
    local bEmpty = true
    for k, v in pairs(tb) do
        ...
        if type(v) == 'table' then
            if nDeep < 4 and not IsBasicTable(v, nDeep + 1) then
                return false
            end
...
function M.infer_values(top_ast, tokenlist, src, report, nPass)
    ...
    LA.walk(top_ast,function(ast, func_depth) -- walk down
        ...
        elseif ast.tag == 'Set' then
            ...
            for i=1,#vars_ast do
                ...
                if value_ast and value_ast.tag == 'Function' then
                    local first_param = value_ast[1][1]
                    if (first_param and first_param.tag == 'Id' and first_param[1] == "self" and var_ast[1].localdefinition) then
                        set_value(first_param, var_ast[1].localdefinition.value)
                    end
                end
            end
    ...
    elseif ast.tag == 'Call' or ast.tag == 'Invoke' then
        ...
        if not found then
            ...
            if mf then
                ast.valuelist = copytable(mf.outputs); ast.value = ast.valuelist[1]
            else
...
-- CATEGORY: code interpretation
function M.inspect(top_ast, tokenlist, src, report, nPass)
    ...
    LA.walk(top_ast, function(ast)
        ...
        elseif ast.tag == 'Call' or ast.tag == 'Invoke' then
            ...
            if LS.argument_counts[value] then -- chenliang3: check native func's params only
...
function M.get_value_details(ast, tokenlist, src, ID_Value_Map)
    ...
    if type(vast.value) == 'table' and type(vast.value["__tostring"]) ~= 'function' then
        ...
        for k, v in pairs(vast.value) do
            k = tostring(k)
            if k:match("^[%w_]+$") then
                table.insert(keys, {Name=k,Type=type(v)})
            end
        end
        if #keys > 0 then
