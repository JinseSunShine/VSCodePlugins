...
-- Check arguments
--
if not (args.spaces or args.tabs) then
    -- Default indentation
    args.spaces = 2
end
...
if not delimiter then
    error( string.format('Unhandled delimiter: %s.', args.delimiter) )
    return
end
...
if #args == 0 then
    ...
    if formatted then
        ...
        error(string.format('Unable to format :\n%s', errormessage))
    end
else
    for _, filename in ipairs(args) do
        ...
        if formatted then
            ...
            error(string.format('Unable to format `%s`:\n%s', filename, errormessage))
        end
    end
end
