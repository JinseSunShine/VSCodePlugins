    ...
    local src_html = annotate_source(src, ast, tokenlist, function(snip_src, token)
        ...
        if token then
            ...
            if token.tag == 'Id' or ast.isfield then
                ...
                local value_details = LI.get_value_details(ast, tokenlist, src)
                local json = require "json"
                for k, v in pairs(value_details) do
                    if (type(v) == 'table') then
                        value_details[k] = json.encode(v)
                    end
                end
                local desc_html = escape_html(table.concat(value_details, "\n"))
